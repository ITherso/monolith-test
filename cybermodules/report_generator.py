    ...
    def gather_scan_data(self):
        ...
        try:
            with db_conn() as conn:
                ...
                self.passive_insights = self._build_passive_insights()
...
Düşük öncelik'''
        ...
        for t in techs:
            try:
                tech = {
                    ...
                    'via': t[3] if len(t) > 3 else 'Unknown',
                    'confidence': t[4] if len(t) > 4 else 0,
                }
    ...
    def _assess_tech_risk(self, tech) -> str:
        ...
        if len(tech) > 4:
            try:
                confidence = int(tech[4])
            except Exception:
                confidence = 0
        
        if confidence and confidence < 50:
            return 'MEDIUM - Low confidence fingerprint'
        if any(x in name for x in ['php', 'wordpress', 'apache', 'nginx', 'openssl']):
            return 'HIGH - Potentially outdated version detected'
        elif any(x in name for x in ['django', 'laravel', 'drupal', 'joomla']):
    ...
    def _build_passive_insights(self) -> List[Dict]:
        """Teknoloji ve istihbarat verilerinden pasif keşif özetleri oluştur"""
        insights = []
        seen = set()

        def add_insight(insight_type: str, title: str, detail: str, severity: str = 'INFO'):
            key = (insight_type, title, detail)
            if key in seen:
                return
            seen.add(key)
            insights.append({
        ...
        for tech in self.technologies:
            ...
            if category in {'Web Server', 'Application Framework', 'CMS/Framework'}:
                detail = f"{name} {version}".strip()
                add_insight('TECH_FINGERPRINT', f'{name} detected', detail, 'INFO')

            lower_name = name.lower()
            if 'wordpress' in lower_name:
                add_insight('CMS', 'WordPress detected', f'Fingerprint matched for {name}', 'INFO')
                if version:
                    add_insight('CMS_VERSION', 'WordPress version exposed', version, 'LOW')
            elif 'drupal' in lower_name:
                add_insight('CMS', 'Drupal detected', f'Fingerprint matched for {name}', 'INFO')
                if version:
                    add_insight('CMS_VERSION', 'Drupal version exposed', version, 'LOW')
            elif 'joomla' in lower_name:
                add_insight('CMS', 'Joomla detected', f'Fingerprint matched for {name}', 'INFO')
                if version:
                    add_insight('CMS_VERSION', 'Joomla version exposed', version, 'LOW')

            if version:
                add_insight('VERSION', f'{name} version', version, 'LOW')

        admin_markers = (
            '/admin', '/wp-admin', '/wp-login', '/administrator', '/login',
            'admin panel', 'administration', 'dashboard', 'signin', 'auth'
        )
        for item in self.intelligence:
            ...
            if item_type in {'NMAP_PORT', 'NMAP_INFO', 'GOBUSTER_FOUND', 'GOBUSTER_SUMMARY', 'NUCLEI_SUMMARY'}:
                add_insight('RECON', item_type.replace('_', ' ').title(), data[:240], 'INFO')

            if any(marker in detail for marker in admin_markers):
                add_insight('ADMIN_SURFACE', 'Administrative surface discovered', data[:240], 'MEDIUM')

            if 'version' in detail or 'generator' in detail:
                add_insight('VERSION_HINT', item_type.replace('_', ' ').title(), data[:240], 'LOW')

        for log in self.tool_logs:
            output = str(log.get('output', ''))
            lower_output = output.lower()
            if any(marker in lower_output for marker in admin_markers):
                add_insight('ADMIN_SURFACE', f"{log.get('tool', 'TOOL')} output", output[:240], 'MEDIUM')
    ...
    def generate_executive_summary(self) -> Dict:
        ...
        passive_count = len(self.passive_insights)
        ...
        return {
            ...
            'passive_insight_count': passive_count,
    ...
    def _generate_narrative_summary(self) -> str:
        ...
        if total == 0:
            passive_note = ""
            if self.passive_insights:
                passive_note = f"\nPasif keşif sırasında {len(self.passive_insights)} adet teknoloji/servis izi ve yüzey bilgisi toplandı."
            return f"""Güvenlik değerlendirmesi, {self.target} hedefinde kritik güvenlik açığı tespit etmedi. 
Standart güvenlik en iyi uygulamaları takip edildi. Ancak, eksik güvenlik header'ları ve potansiyel iyileştirme alanları belirlendi.{passive_note}"""
...
Acil müdahale gerektiren bulgular önceliklendirilmiştir."""
    ...
    def _generate_attack_narrative(self) -> str:
        ...
        if self.passive_insights:
            notable = [i['title'] for i in self.passive_insights[:5]]
            narrative.append(f"   - Pasif yüzey bulguları: {', '.join(notable)}")
...
<body>
    <div class="container">
        ...
        <div class="card">
            <h2 class="card-title">PASSIVE RECONNAISSANCE ({len(self.passive_insights)} items)</h2>
            {self._generate_passive_html()}
        </div>

        <!-- IT Handoff Summary -->
        <div class="card">
            <h2 class="card-title">IT HANDOFF SUMMARY</h2>
            {self._generate_it_handoff_html()}
        </div>
...
"""
    ...
    def _generate_passive_html(self) -> str:
        """Pasif keşif HTML'i oluştur"""
        if not self.passive_insights:
            return '<p style="color: #888;">No passive reconnaissance data collected.</p>'

        html = '<div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(240px, 1fr)); gap: 15px;">'
        for insight in self.passive_insights[:24]:
            severity = insight.get('severity', 'INFO').lower()
            html += f"""
            <div style="background: rgba(0,0,0,0.3); border-radius: 10px; padding: 16px; border: 1px solid rgba(255,255,255,0.08); border-left: 4px solid var(--{severity if severity in ['critical', 'high', 'medium', 'low', 'info'] else 'info'});">
                <div style="color: #00ff88; font-size: 0.8em; font-weight: 700; margin-bottom: 8px;">[{insight.get('type', 'INFO')}]</div>
                <div style="font-weight: 600; margin-bottom: 6px;">{insight.get('title', 'Passive Finding')}</div>
                <div style="color: #ccc; font-size: 0.9em;">{insight.get('detail', '')[:220]}</div>
            </div>
            """
        html += '</div>'
        return html

    def _generate_it_handoff_html(self) -> str:
        """IT teslim özeti HTML'i oluştur"""
        summary = self.generate_executive_summary()
        action_items = [
            'Review any exposed administrative surfaces and restrict public access.',
            'Patch or harden detected technology versions and CMS fingerprints.',
            'Validate login, session, and header controls before retest.',
            'Retest after remediation and archive the report for change tracking.',
        ]

        html = f"""
        <div style="background: rgba(0,0,0,0.3); border-radius: 12px; padding: 18px; border: 1px solid rgba(255,255,255,0.08);">
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 12px; margin-bottom: 16px;">
                <div><strong>Target</strong><br><span style="color:#ccc;">{self.target}</span></div>
                <div><strong>Risk</strong><br><span style="color:#ccc;">{summary['overall_risk']} ({summary['risk_score']}/100)</span></div>
                <div><strong>Findings</strong><br><span style="color:#ccc;">{summary['total_findings']}</span></div>
                <div><strong>Passive Signals</strong><br><span style="color:#ccc;">{summary['passive_insight_count']}</span></div>
            </div>
            <div style="margin-bottom: 14px; color: #ccc; line-height: 1.7;">{self._pdf_safe(summary['summary'])}</div>
            <div style="margin-bottom: 10px; font-weight: 600; color: #00ff88;">Recommended IT actions</div>
            <ul style="margin-left: 20px; color: #ddd; line-height: 1.7;">
                {''.join([f'<li>{self._pdf_safe(item)}</li>' for item in action_items])}
            </ul>
        </div>
        """
        return html

    def _pdf_safe(self, value) -> str:
        """PDF çıktısı için ASCII güvenli metin üret"""
        text = unicodedata.normalize('NFKD', str(value)).encode('ascii', 'ignore').decode('ascii')
        return text.replace('\r', '')

    def _add_pdf_section(self, pdf: FPDF, title: str, lines: List[str]):
        effective_width = pdf.w - pdf.l_margin - pdf.r_margin
        pdf.set_font('Arial', 'B', 13)
        pdf.multi_cell(effective_width, 8, self._pdf_safe(title))
        pdf.ln(1)
        pdf.set_font('Arial', '', 10)
        for line in lines:
            text = self._pdf_safe(line)
            if not text:
                continue
    ...
    def _generate_pdf_report(self, output_path: str) -> bool:
        """Simple text PDF report oluştur"""
        try:
            pdf = FPDF()
            ...
            pdf.multi_cell(effective_width, 10, self._pdf_safe('SECURITY ASSESSMENT REPORT'), align='C')
            pdf.ln(2)
            ...
            meta_lines = [
                ...
                f'Risk Score: {self.risk_score}/100 ({self._get_risk_label()})',
            ]
            self._add_pdf_section(pdf, 'Overview', meta_lines)

            summary = self.generate_executive_summary()
            self._add_pdf_section(pdf, 'Executive Summary', [summary['summary']])

            it_lines = [
                f"Target: {self.target}",
                f"Risk: {summary['overall_risk']} ({summary['risk_score']}/100)",
                f"Findings: {summary['total_findings']}",
                f"Passive Signals: {summary['passive_insight_count']}",
            ...
            self._add_pdf_section(pdf, 'IT Handoff Summary', it_lines)

            vuln_lines = [
                f"Critical: {self.vuln_summary['critical']}",
                f"High: {self.vuln_summary['high']}",
                f"Medium: {self.vuln_summary['medium']}",
                f"Low: {self.vuln_summary['low']}",
                f"Info: {self.vuln_summary['info']}",
            ]
            self._add_pdf_section(pdf, 'Vulnerability Summary', vuln_lines)

            passive_lines = []
            for insight in self.passive_insights[:20]:
                passive_lines.append(f"[{insight.get('severity', 'INFO')}] {insight.get('title', '')}: {insight.get('detail', '')}")
            if not passive_lines:
                passive_lines = ['No passive reconnaissance data collected.']
            self._add_pdf_section(pdf, 'Passive Reconnaissance', passive_lines)

            tech_lines = []
            for tech in self.technologies[:20]:
                tech_lines.append(f"{tech.get('name', 'Unknown')} {tech.get('version', '')} | {tech.get('category', 'Other')} | {tech.get('risk_level', '')}")
            if not tech_lines:
                tech_lines = ['No technology information detected.']
            self._add_pdf_section(pdf, 'Technology Stack', tech_lines)

            finding_lines = []
            for finding in self.findings[:20]:
                finding_lines.append(
                    f"[{finding['severity']}] {finding['type']} | URL: {finding['url'] or 'N/A'} | Component: {finding['affected']}"
                )
                if finding.get('description'):
                    finding_lines.append(f"  Detail: {finding['description'][:220]}")
            if not finding_lines:
                finding_lines = ['No vulnerabilities detected.']
            self._add_pdf_section(pdf, 'Key Findings', finding_lines)

            intel_lines = []
            for item in self.intelligence[:20]:
                intel_lines.append(f"[{item.get('type', 'UNKNOWN')}] {item.get('data', '')}")
            if not intel_lines:
                intel_lines = ['No intelligence data collected.']
            self._add_pdf_section(pdf, 'Scan Intelligence', intel_lines)

            rec_lines = []
            for rec in self._get_prioritized_recommendations()[:10]:
                rec_lines.append(f"[{rec['priority']}] {rec['title']} -> {rec['affected']}")
                if rec.get('description'):
                    rec_lines.append(rec['description'])
            if not rec_lines:
                rec_lines = ['No remediation needed.']
            self._add_pdf_section(pdf, 'Remediation Roadmap', rec_lines)
    ...
    def save_report(self, output_dir: str = "/tmp") -> Dict:
        ...
        # PDF kaydet
        pdf_path = f"{output_dir}/report_{self.scan_id}_{safe_target}.pdf"
        pdf_generated = self._generate_pdf_report(pdf_path)
