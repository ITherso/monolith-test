...
class LateralMovementEngine:
    ...
    def __init__(self, scan_id, session_info=None, opsec_enabled=False):
        ...
        self.domain = self.session_info.get("domain", "")
        self.username = self.session_info.get("username", "")
        self.password = self.session_info.get("password", "")
        self.nt_hash = self.session_info.get("nt_hash", "")
        self.lm_hash = self.session_info.get("lm_hash", "")
        self.target_domain = self.session_info.get("target_domain", "")
        self.targets = []
        self.results = []
        ...
        try:
            with db_conn() as conn:
                for result in self.results:
                    ...
                    for method_result in result.get('methods', []):
                        if method_result.get('success') or not method:
                            method = method_result.get('method', 'unknown')
                            error_msg = method_result.get('error', '')[:500] if not method_result.get('success') else None
                            output = method_result.get('output', '')[:1000]
                    
                    # Determine credentials type
                    creds = result.get('credentials', {})
                    if creds.get('nt_hash') or creds.get('lm_hash'):
                        creds_type = 'HASH'
                    ...
                    # Determine access level (basic heuristic)
                    if success and output:
                        if 'admin' in output.lower() or 'system' in output.lower():
                            access_level = 'ADMIN'
                    ...
                    # Insert to lateral_movement table
                    status = 'SUCCESS' if success else 'FAILED'
                    
                    conn.execute(
                        """INSERT INTO lateral_movement 
                        (scan_id, source_host, target_host, method, username, status, 
                         error_message, command_output, execution_time, credentials_type, 
                         access_level, timestamp) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))""",
                        (
                            self.scan_id,
                            None,  # source_host determined dynamically
                            target,
                            method,
                            username,
                            status,
                    ...
                    # Also log to intel for visibility
                    if success:
                        conn.execute(
                            ...
                            (
                                self.scan_id,
                                "LATERAL_SUCCESS",
                                f"{target} via {method} as {username}"
                            )
                        )
                    else:
                        conn.execute(
                            ...
                            (
                                self.scan_id,
                                "LATERAL_ATTEMPT",
                                f"{target}: {error_msg or 'connection failed'}"
                            )
                        )
