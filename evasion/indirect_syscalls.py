...
def _safe_read_memory(addr: int, size: int) -> bytes:
    """Safely read process memory using ReadProcessMemory to prevent access violations."""
...
class HellsGateResolver:
    ...
    def resolve_ssn(self, func_name: str) -> SyscallEntry:
        ...
        try:
            ...
            stub = _safe_read_memory(func_addr, 32)
            stub_bytes = bytes(stub) if stub else b""
    ...
    def _get_export_address(self, module_base: int, func_name: str) -> int:
        ...
        try:
            # Parse PE headers
            dos_header = _safe_read_memory(module_base, 64)
            
            # Verify DOS signature
            if len(dos_header) < 2 or struct.unpack('<H', dos_header[:2])[0] != IMAGE_DOS_SIGNATURE:
                return 0
            ...
            # Verify PE signature
            pe_sig = _safe_read_memory(pe_header, 4)
            if len(pe_sig) < 4 or struct.unpack('<I', pe_sig[:])[0] != IMAGE_NT_SIGNATURE:
                return 0
            
            # Get export directory RVA (offset 112 in PE64 optional header)
            optional_header = pe_header + 24
            export_rva_bytes = _safe_read_memory(optional_header + 112, 4)
            export_rva = struct.unpack('<I', export_rva_bytes[:])[0] if len(export_rva_bytes) >= 4 else 0
            ...
            num_names_bytes = _safe_read_memory(export_dir + 24, 4)
            num_names = struct.unpack('<I', num_names_bytes[:])[0] if len(num_names_bytes) >= 4 else 0
            
            name_table_rva_bytes = _safe_read_memory(export_dir + 32, 4)
            name_table_rva = struct.unpack('<I', name_table_rva_bytes[:])[0] if len(name_table_rva_bytes) >= 4 else 0
            
            ordinal_table_rva_bytes = _safe_read_memory(export_dir + 36, 4)
            ordinal_table_rva = struct.unpack('<I', ordinal_table_rva_bytes[:])[0] if len(ordinal_table_rva_bytes) >= 4 else 0
            
            addr_table_rva_bytes = _safe_read_memory(export_dir + 28, 4)
            addr_table_rva = struct.unpack('<I', addr_table_rva_bytes[:])[0] if len(addr_table_rva_bytes) >= 4 else 0
            ...
            for i in range(num_names):
                name_rva_bytes = _safe_read_memory(name_table + i * 4, 4)
                name_rva = struct.unpack('<I', name_rva_bytes[:])[0] if len(name_rva_bytes) >= 4 else 0
                name_addr = module_base + name_rva
                
                # Read name string
                name_bytes = _safe_read_memory(name_addr, 64)
                if not name_bytes:
                    continue
                ...
                if name == target_name:
                    # Get ordinal
                    ordinal_bytes = _safe_read_memory(ordinal_table + i * 2, 2)
                    ordinal = struct.unpack('<H', ordinal_bytes[:])[0] if len(ordinal_bytes) >= 2 else 0
                    
                    # Get function address
                    func_rva_bytes = _safe_read_memory(addr_table + ordinal * 4, 4)
                    func_rva = struct.unpack('<I', func_rva_bytes[:])[0] if len(func_rva_bytes) >= 4 else 0
