...
class RoutesExtraTest(unittest.TestCase):
    ...
    def test_scans_page_handles_incomplete_scan_rows(self):
        self._login_admin()
        with db_conn() as conn:
            conn.execute(
                "INSERT INTO scans (target, date, status, user_id) VALUES (?, ?, ?, ?)",
                ("http://incomplete.example", None, None, None),
            )
            conn.commit()
    ...
    def test_dashboard_script_does_not_include_raw_link_in_script(self):
        self._login_admin()
        res = self.client.get("/")
        self.assertEqual(res.status_code, 200)
        self.assertNotIn(b"Distributed Agents Console", res.data)
