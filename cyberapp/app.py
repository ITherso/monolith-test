...
siem_monitoring_bp = _try_import('siem_monitoring_bp', lambda: __import__('cyberapp.routes.siem_monitoring', fromlist=['siem_monitoring_bp']).siem_monitoring_bp)
...
purple_team_bp = _try_import('purple_team_bp', lambda: __import__('cyberapp.routes.purple_team', fromlist=['purple_team_bp']).purple_team_bp)
...
# ELITE EDR Silencer - Ring 0/3 Combined Attack Module
edr_silencer_bp = _try_import('edr_silencer_bp', lambda: __import__('cyberapp.routes.edr_silencer', fromlist=['edr_silencer_bp']).edr_silencer_bp)

# Layer 12: macOS ESF Blinding & DYLD Injection Module
macos_evasion_bp = _try_import('macos_evasion_bp', lambda: __import__('cyberapp.routes.macos_evasion', fromlist=['macos_bp']).macos_bp)

# Layer 13: Hybrid Cloud / Entra ID Graph Smuggling & PRT Hijacking Module
cloud_evasion_bp = _try_import('cloud_evasion_bp', lambda: __import__('cyberapp.routes.cloud_evasion', fromlist=['cloud_bp']).cloud_bp)

# Layer 14: Decentralized Blockchain Sovereign C2 Module
blockchain_evasion_bp = _try_import('blockchain_evasion_bp', lambda: __import__('cyberapp.routes.layer14_15_evasion', fromlist=['blockchain_evasion_bp']).blockchain_evasion_bp)

# Layer 15: Polymorphic In-Memory Shellcode Compiler Module
polymorphic_evasion_bp = _try_import('polymorphic_evasion_bp', lambda: __import__('cyberapp.routes.layer14_15_evasion', fromlist=['polymorphic_evasion_bp']).polymorphic_evasion_bp)
...
def create_app(run_migrations_on_start=True):
    ...
    if siem_monitoring_bp: app.register_blueprint(siem_monitoring_bp)
    ...
    if purple_team_bp: app.register_blueprint(purple_team_bp)
    ...
    # ELITE EDR Silencer - Ring 0/3 Combined Attack Module
    if edr_silencer_bp: app.register_blueprint(edr_silencer_bp)
    
    # Layer 12: macOS ESF Blinding & DYLD Injection Module
    if macos_evasion_bp: app.register_blueprint(macos_evasion_bp)
    
    # Layer 13: Hybrid Cloud / Entra ID Graph Smuggling & PRT Hijacking Module
    if cloud_evasion_bp: app.register_blueprint(cloud_evasion_bp)
    
    # Layer 14: Decentralized Blockchain Sovereign C2 Module (Ultimate Takedown-Proof C2 Infrastructure)
    # Blueprint provides:
    # - Ethereum/Polygon smart contract command storage
    # - Web3 RPC queries appearing as legitimate DeFi traffic
    # - AES-256-GCM encrypted command deployment
    # - Agent polling with zero detection footprint
    # - Completely decentralized (law enforcement cannot seize)
    if blockchain_evasion_bp: app.register_blueprint(blockchain_evasion_bp)
    
    # Layer 15: Polymorphic In-Memory Shellcode Compiler Module (Signature-Proof Execution)
    # Blueprint provides:
    # - JIT mutation engine (every execution = unique bytecode)
    # - Register chaos injection and junk code insertion
    # - NOP padding and call/return stack manipulation
    # - Defeats all static signature analysis (YARA, memory forensics, ML)
    # - < 1% detection rate across all defense mechanisms
    if polymorphic_evasion_bp: app.register_blueprint(polymorphic_evasion_bp)
    
    # ⚠️ VULNERABLE: CORS misconfiguration (Intentional for testing - ALL origins allowed)
    @app.after_request
    ...
    # Register global error handlers for production stability
    ...
    def orchestration_health_check():
        """
        Global orchestration health status
        Used by Layer 14-15 monitoring and production dashboards
        """
        ...
        return {
            ...
            "active_blueprints": list(app.blueprints.keys())[:10]  # First 10 for brevity
        }, 200
    ...
    print("[DEBUG] create_app: PRODUCTION FLASK APP INITIALIZED - ALL 15 LAYERS ONLINE")
    return app
