...
def _store_intel(scan_id, item_type, data):
    try:
        with db_conn() as conn:
            conn.execute(
                "INSERT INTO intel (scan_id, type, data) VALUES (?, ?, ?)",
                (scan_id, item_type, data),
            )
            conn.commit()
    except Exception:
        pass
...
def _store_techno(scan_id, name, detected_via, confidence=80):
    try:
        with db_conn() as conn:
            conn.execute(
                "INSERT INTO techno (scan_id, name, detected_via, confidence) VALUES (?, ?, ?, ?)",
                (scan_id, name, detected_via, confidence),
            )
            conn.commit()
    except Exception:
        pass
...
def _store_vuln(scan_id, vuln_type, url, fix, severity):
    try:
        with db_conn() as conn:
            conn.execute(
                "INSERT INTO vulns (scan_id, type, url, fix, severity) VALUES (?, ?, ?, ?, ?)",
                (scan_id, vuln_type, url, fix, severity),
            )
            conn.commit()
    except Exception:
        pass
...
def _perform_passive_web_scan(target, scan_id):
    """Perform a lightweight HTTP-only scan when native tools are missing."""
    ...
    headers = {
        "User-Agent": "MonolithPassiveScanner/1.0",
    ...
    try:
        ...
        with urllib_request.urlopen(req, timeout=12, context=context) as resp:
            status = getattr(resp, "status", resp.getcode())
            response_headers = {k.lower(): v for k, v in resp.headers.items()}
            body = resp.read(200000).decode("utf-8", errors="ignore")
    except Exception as exc:
        _log_tool_output(scan_id, "PASSIVE_SCAN", f"Passive scan failed: {str(exc)[:200]}")
        return

    title = _extract_title(body)
    _store_intel(scan_id, "PASSIVE_STATUS", f"HTTP {status}")
    _store_intel(scan_id, "PASSIVE_TITLE", title or "No title detected")
    _store_intel(scan_id, "PASSIVE_HEADERS", ", ".join(sorted(response_headers.keys())[:25]))

    server = response_headers.get("server", "")
    powered_by = response_headers.get("x-powered-by", "")
    if server:
        _store_techno(scan_id, server[:120], "PASSIVE_SERVER_HEADER", 95)
        _store_intel(scan_id, "PASSIVE_SERVER", server[:240])
    if powered_by and powered_by != server:
        _store_techno(scan_id, powered_by[:120], "PASSIVE_X_POWERED_BY", 90)
        _store_intel(scan_id, "PASSIVE_POWERED_BY", powered_by[:240])

    if title:
        lower_title = title.lower()
        if "wordpress" in lower_title:
            _store_techno(scan_id, "WordPress", "PASSIVE_TITLE_FINGERPRINT", 70)
        elif "drupal" in lower_title:
            _store_techno(scan_id, "Drupal", "PASSIVE_TITLE_FINGERPRINT", 70)
        elif "joomla" in lower_title:
            _store_techno(scan_id, "Joomla", "PASSIVE_TITLE_FINGERPRINT", 70)
        else:
            _store_techno(scan_id, title[:120], "PASSIVE_TITLE", 55)

    missing_headers = []
    header_checks = {
        "content-security-policy": ("Content-Security-Policy", "Add a restrictive CSP header.", "LOW"),
        "x-frame-options": ("X-Frame-Options", "Add X-Frame-Options to mitigate clickjacking.", "LOW"),
        "x-content-type-options": ("X-Content-Type-Options", "Add X-Content-Type-Options: nosniff.", "LOW"),
        "referrer-policy": ("Referrer-Policy", "Set a strict Referrer-Policy.", "LOW"),
    }
    if parsed.scheme == "https":
        header_checks["strict-transport-security"] = (
            "Strict-Transport-Security",
            "Add HSTS for HTTPS targets.",
            "LOW",
        )

    for header_key, (display_name, remediation, severity) in header_checks.items():
        if not response_headers.get(header_key):
            missing_headers.append(display_name)
            _store_vuln(
                scan_id,
                f"MISSING_{display_name.upper().replace('-', '_').replace(' ', '_')}",
                target,
                remediation,
                severity,
            )
    ...
    if missing_headers:
        _store_intel(scan_id, "PASSIVE_MISSING_HEADERS", ", ".join(missing_headers))
    ...
    if parsed.scheme == "http":
        _store_vuln(
            ...
            "PLAINTEXT_TRANSPORT",
            target,
            "Serve the application over HTTPS to protect credentials and session data.",
            "MEDIUM",
        )

    robots_url = f"{parsed.scheme}://{parsed.netloc}/robots.txt"
    try:
        req = urllib_request.Request(robots_url, headers=headers)
        with urllib_request.urlopen(req, timeout=8, context=context) as resp:
            if getattr(resp, "status", resp.getcode()) == 200:
                _store_intel(scan_id, "PASSIVE_ROBOTS", "robots.txt is accessible")
    except urllib_error.HTTPError:
        pass
    except Exception:
        pass

    _log_tool_output(scan_id, "PASSIVE_SCAN", f"Passive scan completed with {len(missing_headers)} missing headers")
    ...
    try:
        ...
        if selected_tools:
            ...
            try:
                ...
                arsenal = SupremeArsenalEngine(target, scan_id, selected_tools, user_id)
                arsenal.start()
            except ImportError:
        ...
        if vuln_count == 0:
            _perform_passive_web_scan(target, scan_id)
